...
export const FoodListPage = ({ title }: { title: string }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    apiUtil({
      params: { q: search, published: published },
    }).then((data) => {
      setFoodList(data);
    });
  ...
  const handleDelete = async (foodItem: Ingredient) => {
    const confirmDelete = await new Promise((resolve) => {
      promptDialog({
        ...
        footButton: [
          ...
          { onClick: () => resolve(true), children: 'Delete' },
        ],
    ...
    apiUtil({ pathParams: { id: foodItem.id } })
      .then(() => {
        fetchData();
      })
      .catch((err: APIError) => {
        if (err.status === 409)
          promptDialog({
            ...
            footButton: [{ type: 'hollow', children: 'OK' }],
          });
      });
  };
