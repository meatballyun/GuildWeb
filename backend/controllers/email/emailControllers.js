...
class MailController {
  async sendSignUp(req, res, next) {
    try {
      const confirmationMail = await ConfirmationEmail.getConfirmationEmailsByUserId(
        req.body.uid,
        'SignUp'
      );
      ...
      transporter.sendMail(signUpEmail(req.body.email, req.body.uid, CODE), function (err, info) {
        if (err) return next(new ApplicationError(404, 'Email address not found.'));
    ...
    try {
      ...
      if (!user?.length)
        return next(
          new ApplicationError(
            404,
            'The email address you clicked on has not been registered with any account.'
          )
        );
      ...
      if (query.STATUS === 'Confirmed')
        return next(
          new ApplicationError(
            403,
            'This email address has already been verified. Your account is already activated.'
          )
        );
  ...
  async validationResetPassword(req, res, next) {
    try {
      ...
      if (confirmationMail.CODE === req.query.code) {
        const query = await ConfirmationEmail.updateConfirmationEmail(
          req.query.uid,
          'Confirmed',
          'ForgotPassword'
        );
      ...
      return next(
        new ApplicationError(404, 'The provided confirmation code does not exist or has expired.')
      );
    } catch (err) {
      return next(new ApplicationError(400, err));
    }
    ...
    try {
      ...
      if (confirmationMail.STATUS === 'Confirmed')
        return next(
          new ApplicationError(
            403,
            'The verification link has expired or the account is already activated.'
          )
        );
      if (confirmationMail.CODE === req.query.code) {
        const query = await ConfirmationEmail.updateConfirmationEmail(
        ...
        if (query.affectedRows) {
          ...
          if (rows.affectedRows)
            return res.status(200).json({
              success: true,
              message:
                'The provided confirmation code is valid and can be used for user validation.',
              data: 'OK',
            });
          return next(new ApplicationError(404, 'User not found.'));
        }
