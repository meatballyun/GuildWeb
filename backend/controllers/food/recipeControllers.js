...
class RecipeController {
    async getRecipes(req, res, next) {
        try {
            ...
            if ( req.query.published ){
                const publicRecipes = req.query.q ? await Recipe.getRecipesByName(req.session.passport.user, req.query.q) : await Recipe.getRecipes(req.session.passport.user);
    ...
    async addRecipe(req, res, next) {
        try {        
            ...
            const newRecipe = await Recipe.addRecipe(req.session.passport.user, req.body.name, req.body.description, req.body.carbs, req.body.pro, req.body.fats, req.body.kcal, req.body.unit, req.body.imageUrl, req.body.published);
            
            await Promise.all(req.body.ingredients.map(async (ingredient)=>{
                const [q] = await Ingredient.getIngredientsById(ingredient.id);
                if (q.CREATOR === req.session.passport.user) await RecipeIngredientRelation.addRecipeIngredientRelation(ingredient.id, newRecipe['insertId'], ingredient.amount); 
                else{                    
                    const copyIngredient = await Ingredient.addIngredient(req.session.passport.user, q.NAME, q.DESCRIPTION, q.CARBS, q.PRO, q.FATS, q.KCAL, q.UNIT, q.IMAGE_URL, false);
                    await RecipeIngredientRelation.addRecipeIngredientRelation(copyIngredient['insertId'], newRecipe['insertId'], ingredient.amount);
                };
            }));

            if(req.body.published){
                const relations = await RecipeIngredientRelation.getRecipeIngredientRelationByRecipe(newRecipe['insertId']);
    ...
    async updateRecipe(req, res, next) {
        try {
            ...
            await Promise.all(req.body.ingredients.map(async (ingredient)=>{
                const getRelations = await RecipeIngredientRelation.getRecipeIngredientRelationByIngredientAndRecipe(ingredient.id, req.params.id);
                if (getRelations?.length) await RecipeIngredientRelation.updateRecipeIngredientRelation(ingredient.id, req.params.id, ingredient.amount);
                else {
                    const [q] = await Ingredient.getIngredientsById(ingredient.id);
                    if (q.CREATOR === req.session.passport.user) await RecipeIngredientRelation.addRecipeIngredientRelation(ingredient.id, req.params.id, ingredient.amount); 
                    else{
                        const copyIngredient = await Ingredient.addIngredient(req.session.passport.user, q.NAME, q.DESCRIPTION, q.CARBS, q.PRO, q.FATS, q.KCAL, q.UNIT, q.IMAGE_URL, false);
                        await RecipeIngredientRelation.addRecipeIngredientRelation(copyIngredient['insertId'], req.params.id, ingredient.amount);
                    };
                }
            }));

            if(req.body.published){
                const relations = await RecipeIngredientRelation.getRecipeIngredientRelationByRecipe(req.params.id);
                relations.map(async (relation)=>{
                    await Ingredient.publishIngredient(relation.INGREDIENTS );
                })
            }
