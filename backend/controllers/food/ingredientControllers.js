...
class IngredientController {
  async getIngredients(req, res, next) {
    try {
      ...
      ingredients = req.query.q
        ? await Ingredient.getIngredientsByCreatorAndName(req.session.passport.user, req.query.q)
        : await Ingredient.getIngredientsByCreator(req.session.passport.user);

      if (req.query.published) {
        const publicIngredients = req.query.q
          ? await Ingredient.getIngredientsByName(req.session.passport.user, req.query.q)
          : await Ingredient.getIngredients(req.session.passport.user);
  ...
  async addIngredient(req, res, next) {
    try {
      const newIngredient = await Ingredient.addIngredient(
  ...
  async updateIngredient(req, res, next) {
    try {
      const relations = await RecipeIngredientRelation.getRecipeIngredientRelationByIngredient(
        req.params.id
      );
          ...
          if (recipe.PUBLISHED)
            return next(
              new ApplicationError(
                409,
                'Unable to set ingredient as private. It is referenced by a public recipe.'
              )
            );
        }
      }
      ...
      if (ingredient.CREATOR === req.session.passport.user)
        return next(new ApplicationError(409, 'Your are not creator of this ingredient.'));

      const query = await Ingredient.updateIngredient(
        req.params.id,
        req.body.name,
        req.body.description,
        req.body.carbs,
