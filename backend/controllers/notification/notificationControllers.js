...
class NotificationController {
    async getNotifications(req, res, next){
        try{
            ...
            if (notifications && notifications?.length){
                data = await Promise.all(notifications.map(async (row)=>{
                    let sender;
                    if(row.TYPE === 'Guild') {
    ...
    async addNotification(req, res, next){
        try{
            ...
            const newNotification = await Notification.addNotification(req.body.senderId, req.body.recipientId, content.TITLE, content.DESCRIPTION, req.body.type);            
