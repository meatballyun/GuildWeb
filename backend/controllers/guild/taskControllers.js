...
class TaskController {
  ...
  async getTasks(req, res, next) {
    try {
      const tasks = req.query.q
        ? await Task.getTaskByGuildAndName(req.params.gid, req.query.q)
        : await Task.getTaskByGuild(req.params.gid);
      let data;
      if (tasks?.length) {
        data = await Promise.all(
          tasks.map(async (row) => {
            const query = await Adventurer.getAdventurerByTaskAndUser(
              row.ID,
  ...
  async acceptTask(req, res, next) {
    try {
      const [task] = await Task.getTaskDetailById(req.params.tid);
      const [isAdventurer] = await Adventurer.getAdventurerByTaskAndUser(
        req.params.tid,
        req.session.passport.user
      );
      if (!task || !task.ID) {
        ...
        return next(
          new ApplicationError(
            409,
            'The maximum number of adventurers for this task has been reached.'
          )
        );
      }
        ...
        if (req.body.items) {
          await Promise.all(
            req.body.items.map(async (i) => {
              if (i.content) {
                i.id
                  ? await Item.updateItem(i.id, i.content)
                  : await Item.addItem(req.params.tid, i.content);
              } else {
      ...
      if (adventurers && adventurers?.length) {
        await Promise.all(
          adventurers.map(async (i) => {
            const adventurer = await Adventurer.getAdventurerByTaskAndUser(
              req.params.tid,
              i.USER_ID
            );
  ...
  async cancelTask(req, res, next) {
    try {
      ...
      if (!cancelTask.affectedRows)
        return next(new ApplicationError(400, 'Error in Task.cancelTask().'));
      return res.status(200).json({
        success: true,
        message: 'Data update successfully.',
        data: 'OK',
  ...
  async restoreTask(req, res, next) {
    try {
      ...
      if (!restoreTask.affectedRows)
        return next(new ApplicationError(400, 'Error in Task.restoreTask().'));
      return res.status(200).json({
        success: true,
        message: 'Data update successfully.',
        data: 'OK',
  ...
  async submitTask(req, res, next) {
    try {
      ...
      if (!adventurer)
        return next(new ApplicationError(409, 'The task has not been accepted yet.'));
      const currentDate = new Date();
      const query = await Adventurer.updateAdventurerByTaskAndUser(
        req.params.tid,
        req.session.passport.user,
        'Completed',
        currentDate
