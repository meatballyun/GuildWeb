...
class TaskTemplateController {
  async getTaskTemplates(req, res, next) {
    try {
      const taskTemplates = (req.query.q) ? await TaskTemplate.getTaskTemplateByGuildIdAndName(req.params.gid, req.query.q) : await TaskTemplate.getTaskTemplateByGuildId(req.params.gid);
      let data;
      if (taskTemplates?.length){
        data = await Promise.all( taskTemplates.map( async (row) => {
          return {
            id: row.ID,
            enabled: row.ENABLED,
  ...
  async getTaskTemplateDetail(req, res, next) {
    try {
      ...
      if (taskTemplate && taskTemplate.ID){
        ...
        let items = [];
        if (query?.length) {
          items = await Promise.all(query.map(async (row) => {
            return {
              id: row.ID ,
              content: row.CONTENT
            };
  ...
  async addTaskTemplate(req, res, next) {
    try {
      ...
      const newTemplate = await TaskTemplate.addTaskTemplate(req.session.passport.user, req.params.gid, req.body.name, req.body.description,  generationTime, deadline, req.body.type, req.body.maxAdventurer);
  ...
  async updateTaskTemplate(req, res, next) {
    try {
      ...
      } else if (req.member[0].MEMBERSHIP === "Vice" && req.session.passport.user !== taskTemplateDetail[0].CREATOR_ID){
        return next(new ApplicationError(403, "Only guild Master have permission to access this resource."));
      }

      const  generationTime = (new Date(req.body. generationTime)).toISOString().replace('T', ' ').replace('Z', '');
      const deadline = (new Date(req.body.deadline)).toISOString().replace('T', ' ').replace('Z', ''); 
      if ( generationTime > deadline){      
        return next(new ApplicationError(409, "Deadline cannot be earlier than  generationTime."));
      }; 

      const taskTemplate = await TaskTemplate.updateTaskTemplate(req.params.ttid, req.body.enabled, req.body.name, req.body.description,  generationTime, deadline, req.body.type, req.body.maxAdventurer);
      if (taskTemplate.affectedRows){
        if (req.body.items) {
          await Promise.all((req.body.items).map( async(i) => {
            if (i.content){
              (i.id) ? await TemplateItem.updateTemplateItem(i.id , i.content) : await TemplateItem.addTemplateItem(req.params.ttid , i.content);
            } else {
              console.log(i.id);
  ...
  async deleteTaskTemplate(req, res, next) {
    try {
      ...
      } else if (req.member[0].MEMBERSHIP === "Vice" && req.session.passport.user !== taskTemplate[0].CREATOR_ID){
  ...
  async autoBuildTask() {
    ...
    const taskTemplates = await TaskTemplate.getTaskTemplate().catch((err)=>console.log('getTaskTemplate error', err));
    ...
    taskTemplates.map(async(template)=>{
      if (template.GENERATION_TIME < currentTime){
        ...
        const newTask = await Task.addTask(template.CREATOR_ID, template.GUILD_ID, template.NAME, template.GENERATION_TIME, template.DEADLINE, template.DESCRIPTION, template.TYPE, template.MAX_ADVENTURER).catch((err)=>console.log('addTask error', err));
        if (newTask?.['insertId']){
          const templateItems = await TemplateItem.getTemplateItem(template.ID).catch((err)=>console.log('getTemplateItem error', err));
          if (templateItems?.length) {
            await Promise.all(templateItems.map( async(item) => {
              await Item.addItem(newTask['insertId'] , item.CONTENT).catch((err)=>console.log('addItem error', err));
        ...
        const  generationTime = await TaskTemplate.DATE_ADD(template.GENERATION_TIME, 1, unit).catch((err)=>console.log('generationTime.DATE_ADD error', err));
        const  deadline = await TaskTemplate.DATE_ADD(template.DEADLINE, 1, unit).catch((err)=>console.log('deadline.DATE_ADD error', err));
        await TaskTemplate.updateTaskTemplateTime(template.ID, Object.values(generationTime[0])[0], Object.values(deadline[0])[0]).catch((err)=>console.log('updateTaskTemplateTime error', err));
      }
    })
