...
const RepetitiveTask = require('../../models/taskTemplateModel.js');
...
class GuildController {
  async getGuilds(req, res, next) {
    try {
      const query = req.query.q
        ? await UserGuildRelation.getUserGuildRelationByUserAndName(
            req.session.passport.user,
            req.query.q
          )
        : await UserGuildRelation.getUserGuildRelationByUser(req.session.passport.user);
      let guilds = [];
      if (query?.length) {
        guilds = await Promise.all(
          query.map(async (row) => {
            const [guild] = await Guild.getGuild(row.GUILD_ID);
            const [userGuildRelation] = await UserGuildRelation.getUserGuildRelationByGuildAndUser(
              req.session.passport.user,
              row.GUILD_ID
            );
            return {
  ...
  async addGuild(req, res, next) {
    try {
      const newGuild = await Guild.addGuild(
        req.session.passport.user,
        req.body.name,
      ...
      if (newGuild['insertId']) {
        const newUserGuildRelation = await UserGuildRelation.addUserGuildRelation(
          req.session.passport.user,
          newGuild['insertId'],
          'Master'
        );
  ...
  async updateGuild(req, res, next) {
    try {
      const guild = await Guild.updateGuild(
        req.params.gid,
        req.body.name,
        req.body.description,
        req.body.imageUrl
    ...
    try {
      ...
      if (tasks && tasks?.length) {
        tasks.map(async (row) => {
          ...
          if (taskDetail[0].TYPE === 'Repetitive')
            await RepetitiveTask.deleteRepetitiveTask(row.ID);
...
class UserGuildRelationController {
  async replyInvitation(req, res, next) {
    try {
      const member = await UserGuildRelation.getUserGuildRelationByGuildAndUser(
        req.session.passport.user,
        req.params.gid
      );
      if (!member?.length) {
        return next(
          new ApplicationError(409, 'An error occurred while processing your invitation.')
        );
      }
      if (member[0].MEMBERSHIP !== 'Pending') {
        return next(
          new ApplicationError(
            410,
            'You are already a member of this guild. The invitation cannot be accepted.'
          )
        );
      }
      const query = await UserGuildRelation.updateUserGuildRelations(
        req.session.passport.user,
        req.params.gid,
        'Regular'
      );
      ...
      if (getGuildMembers?.length) {
        guildMembers = await Promise.all(
          getGuildMembers.map(async (row) => {
            ...
            const membership = await UserGuildRelation.getUserGuildRelationByGuildAndUser(
              user.ID,
              req.params.gid
            );
            return {
    ...
    try {
      ...
      if (member?.length) {
        return next(
          new ApplicationError(
            409,
            'The player has already been invited to join this guild. Please do not resend the invitation.'
          )
        );
      }
      const newmember = await UserGuildRelation.addUserGuildRelation(
        req.body.uid,
        req.params.gid,
        'Pending'
      );
    ...
    try {
      ...
      if (!query?.length)
        return next(
          new ApplicationError(
            404,
            "The member you are trying to edit does not exist in the guild. Please check the member's information and try again."
          )
        );
    } catch (err) {
      return next(new ApplicationError(400));
  ...
  async deleteMember(req, res, next) {
    try {
      ...
      if (isMaster && isCurrentUser) {
        return next(
          new ApplicationError(403, 'Guild leader cannot delete themselves out of the guild.')
        );
      }
      if (isMaster || isCurrentUser) {
        const query = await UserGuildRelation.deleteUserGuildRelations(
          req.params.uid,
          req.params.gid
        );
        if (query['affectedRows']) {
      ...
      return next(
        new ApplicationError(403),
        'Only guild Master have permission to access this resource.'
      );
    } catch (err) {
      return next(new ApplicationError(400));
    }
