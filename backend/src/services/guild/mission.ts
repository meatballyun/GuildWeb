...
export class MissionService {
  ...
  static async getOne({ tid: missionId }: { tid: number }, uid: number) {
  ...
  static async create({ initiationTime, deadline, items, ...otherData }: MissionDetailed, guildId: number, uid: number) {
  ...
  static async accept({ tid: missionId }: { tid: number }, uid: number) {
    const mission = await MissionModel.getOne(missionId);
  ...
  static async submit({ tid: missionId }: { tid: number }, uid: number) {
    const mission = await MissionModel.getOne(missionId);
    if (!mission) throw new ApplicationError(404);
  ...
  static async abandon({ tid: missionId }: { tid: number }, uid: number) {
    const mission = await MissionModel.getOne(missionId);
    if (!mission) throw new ApplicationError(404);
    const isAccepted = await AdventurerService.isAdventurer(missionId, uid);
    if (!isAccepted) throw new ApplicationError(409);
  ...
  static async complete({ tid: missionId }: { tid: number }, membership: Membership, uid: number) {
    const mission = await MissionModel.getOne(missionId);
    if (!mission) throw new ApplicationError(404);
  ...
  static async fail({ tid: missionId }: { tid: number }, membership: Membership, uid: number) {
    const mission = await MissionModel.getOne(missionId);
    if (!mission) throw new ApplicationError(404);
    if (membership === 'vice' && uid !== mission.creatorId) throw new ApplicationError(403);
  ...
  // prettier-ignore
  static async update({ initiationTime, deadline, items, ...otherData }: MissionDetailed, { tid: missionId }:{ tid: number }, membership: Membership, uid: number) {
    const mission = await MissionModel.getOne(missionId);
    if (!mission) throw new ApplicationError(404);
    if (membership === 'vice' && uid !== mission.creatorId) throw new ApplicationError(403);
  ...
  static async cancel({ tid: missionId }: { tid: number }, membership: Membership, uid: number) {
    const mission = await MissionModel.getOne(missionId);
    if (!mission) throw new ApplicationError(404);
    if (membership === 'vice' && uid !== mission.creatorId) throw new ApplicationError(403);
  ...
  static async restore({ tid: missionId }: { tid: number }, membership: Membership, uid: number) {
    const mission = await MissionModel.getOne(missionId);
    if (!mission) throw new ApplicationError(404);
    if (membership === 'vice' && uid !== mission.creatorId) throw new ApplicationError(403);
  ...
  static async delete({ tid: missionId }: { tid: number }, membership: Membership, uid: number) {
    const mission = await MissionModel.getOne(missionId);
    if (!mission) throw new ApplicationError(404);
    if (membership === 'vice' && uid !== mission.creatorId) throw new ApplicationError(403);

    await AdventurerModel.deleteByMission(missionId);
    await ItemService.delete(missionId);
