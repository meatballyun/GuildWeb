...
class DietRecordModel {
    ...
    static getDietRecord(CREATOR, DIET_DATE) {
        return new Promise((resolve, reject) => {
            connection.query('SELECT * FROM dietRecords WHERE CREATOR = ? AND DIET_DATE = ? AND ACTIVE = TRUE', [CREATOR, DIET_DATE], function (err, rows) {
                if (err) {
                    reject(err);
                } else {
                    resolve(rows);
    ...
    static deleteDietRecord(ID) {
        return new Promise((resolve, reject) => {
            connection.query('UPDATE dietRecords SET ACTIVE = FALSE WHERE ID = ?', [ID], function (err, rows) {
                if (err) {
                    reject(err);
                } else {
                    resolve(rows);
